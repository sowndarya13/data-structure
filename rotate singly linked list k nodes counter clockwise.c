//rotate singly linked list k nodes counter clockwise
#include<stdio.h>
#include<stdlib.h>
struct node
{
 ...
 void rotatelist(int k)
{
   ...
   if(k>=count)
      printf("rotation not possible as nodes are fewer are equal to k\n");
 else
   {
    count=0;
    last->next=head;
...
case 7:display();
       break;
case 8:if(isempty())
          printf("list empty rotation not possible\n");
        else
        {
            printf("enter the k node u want to rotate:\n");
            scanf("%d",&n);
            rotatelist(n);
        }
        break;
