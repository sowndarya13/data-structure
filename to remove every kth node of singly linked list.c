//to remove every kth node of singly linked list
#include<stdio.h>
#include<stdlib.h>
struct node
{
 ...
 void removeknode(int k)
{
   ...
   while(temp!=NULL)
   {
     ...
     printf("there are only few nodes\n");
   }
}
void display()
{
...
case 7:display();
       break;
case 8:if(isempty())
          printf("list empty remove not possible\n");
        else
        {
            printf("enter the k nodes u want to remove:\n");
            scanf("%d",&n);
            removeknode(n);
        }
        break;
