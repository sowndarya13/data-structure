...
temp=head;
if(isempty())
  printf("the list is empty so odd numbers can't be find out\n");
else if(head->next==NULL)
{
  if(head->data%2!=0)
  {
    count++;
    printf("The odd num in list is %d\n",head->data);
   }
   else
    printf("oops!no odd num in list\n");
}
else
{
...
if(count==0)
  printf("oops!there is no odd num in the given list\n");
else
    printf("is/are the odd num(s) in linked list\n");
}
}
void display()
{
