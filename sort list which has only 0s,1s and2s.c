//sort the list with data 0s,1s and
...
void countsort()
{
  int c=head->data,i=0;
  if(head->next==NULL)
     printf("only one element in list .so no swap take place:\n");
  else
  {
  temp=head->next;
  while(temp!=NULL)
...
case 8:if(head!=NULL)
          ...
          printf("sort not possible as the list is empty\n");
       break;
case 9:exit(1);
       break;
